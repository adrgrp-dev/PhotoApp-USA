...
$res=mysqli_query($con,"INSERT INTO `home_seller_info` (`name`, `address`, `mobile_number`, `email`, `city`, `state`,`country`, `zip`,`reference_number`, `contact_person_name`, `contact_person_mobile`, `contact_person_email`,`lead_from`,`request_name`, `request_contact_no`, `request_email`, `request_address`, `request_employer_id`) VALUES ('$sell_name', '$address', '$mobile_no', ' $email_id ', '$city', '$state','US', '$zip', '$ref_no', '$name', '$mobile_no1', '$email_id1','$lead_from','$realtor_name', '$realtor_contactNo','$realtor_email','$realtor_address','$realtor_employer_id')");
...
$res=mysqli_query($con,"UPDATE `home_seller_info` SET `name`='$sell_name',`address`='$address',`mobile_number`='$mobile_no',`email`= ' $email_id ',`city`='$city',`state`='$state',`country`='US',`zip`='$zip',`reference_number`='$ref_no',`contact_person_name`='$name',`contact_person_mobile`='$mobile_no1',`contact_person_email`='$email_id1',`notes`=' ',`lead_from`='$lead_from',`request_name`='$realtor_name',`request_contact_no`='$realtor_contactNo',`request_email`='$realtor_email',`request_address`='$realtor_address',`request_employer_id`='$realtor_employer_id' where id='$hs_id_is'");
...
function get_states(cityIs,stateIs)
{
 ...
 var split=this.responseText;
    document.getElementById("state").innerHTML = split;

    }
  xhttp.open("GET", "../getState.php?city="+cityIs+"&state="+stateIs, true);
  xhttp.send();
  }
  else
  {
...
function validateAddress()
{
    ...
    if(locationTextField=="")
    {
      ...
      $('#locationTextField').val('United States');
      return false;
    }
}
...
?>
    ...
    <div class="col-md-6">
      ...
      <select name="state" class="form-control form-value" id="state" required="" <?php if(@$_REQUEST['u']) { echo "readonly"; } ?>>
       <?php if($appointment_update_details['state']!='') { ?><option value="<?php echo  @$appointment_update_details['state'];?>"><?php echo  @$appointment_update_details['state'];?></option><?php } else{ ?>  <option value="" disabled selected>Select State</option><?php } ?>
                    </select>
      </div>
     <div class="col-md-6">
                        <p id="label_zip_code" adr_trans="label_zip_code">ZIP CODE</p>
                        <input id="zip" name="zip" placeholder="Zip code" type="text" maxlength="12"
 autocomplete="0" class="form-control form-value"  value="<?php echo  @$appointment_update_details['zip'];?>" required=""  >
                    </div>
...
<?php } ?>
	 ...
	 ?>
               &nbsp;&nbsp;<a class="anima-button circle-button btn-sm btn adr-cancel" href="subcsrOrder_list1.php"  id="label_cancel" onclick="return confirm('Are you sure want to cancel the order?');" adr_trans="label_cancel"><i class="fa fa-times"></i>Cancel</a>

			   <?php } else { ?>
			     &nbsp;&nbsp;<a class="anima-button circle-button btn-sm btn adr-cancel" id="label_cancel" adr_trans="label_cancel" onclick="return confirm('Are you sure want to cancel the order?');" href="superorder_list1.php"><i class="fa fa-times"></i>Cancel</a>
...
if (@$_REQUEST['u']==1) {
  ?>
  <script>
var selectedState="<?php echo  @$appointment_update_details['state'];?>";

     get_states("<?php echo  @$appointment_update_details['city'];?>",selectedState);
